  ...
  return (
    ...
    <div>
      ...
      <div className="relative h-60 sm:h-80 flex items-end overflow-hidden bg-earth">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 pb-8 w-full">
          <p className="text-[10px] uppercase tracking-widest text-ivory/60 font-medium mb-1">
            <Link href="/recipes" className="hover:text-ivory transition-colors">
              Recipes
            </Link>
            {' / '}
            {recipe.category}
          </p>
          <h1 className="font-serif text-3xl sm:text-4xl text-ivory">{recipe.title}</h1>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <span
            className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${DIFF_COLOR[recipe.difficulty]}`}
          >
            {recipe.difficulty}
          </span>
          {[
            ...
            <div className="bg-white rounded-2xl border border-earth/10 p-5 lg:sticky lg:top-24">
              ...
              <ul className="space-y-2">
                {recipe.ingredients.map((ing, i) => (
                  <li
                    key={i}
                    className={`text-sm ${ing.startsWith('—') ? 'text-earth/40 text-[11px] uppercase tracking-widest pt-2 font-medium' : 'text-earth/70 flex items-start gap-2'}`}
                  >
                    {!ing.startsWith('—') && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {others.map((r) => (
              ...
              >
                ...
                <div className="p-4">
                  <p className="font-serif text-earth text-sm group-hover:text-terra transition-colors">
                    {r.title}
                  </p>
                  <p className="text-[11px] text-earth/40 mt-0.5">
                    {r.totalTime} · Serves {r.servings}
                  </p>
                </div>
              </Link>
            ))}
